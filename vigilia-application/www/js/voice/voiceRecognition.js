...
class VoiceRecognition {
    ...
    constructor(options = {}) {
        ...
        this.config = {
            ...
            apiKey: null, // API key is optional and can be set via environment or options
     ...
     */
    _detectPlatform() {
        this._log('Detecting platform... typeof cordova ' + typeof cordova, 'info');
    ...
    _initElements() {
        ...
        if (this.elements.micButton) {
            this.elements.micButton.onclick = this._boundMethods.setupVoiceRecognition;
    ...
    async _setupWebRecognition() {
        //this._log('Setting up Web Speech API recognition', 'info');
        ...
        //this._log('Web Speech API supported', 'info');
        ...
        //this._log(`Recognition language set to ${this.state.recognition.lang}`, 'info');
        this.state.recognition.onresult = (event) => {
            const transcript = Array.from(event.results)
                .map(result => result[0].transcript)
                .join(' ')
                .trim();
            const confidence = Math.min(Math.max(event.results[0][0].confidence, this.config.minConfidence), this.config.maxConfidence);
        ...
        this.state.recognition.onstart = () => {
            ...
            this._log('Voice recognition started', 'info');
        };

        //this._log('Setting up recognition event handlers', 'info');
        this.state.recognition.onend = () => {
            this.state.isListening = false;
            this._log('Voice recognition stopped', 'info');
            // Only restart if last error was not aborted/audio-capture
            if (
    ...
    async _setupCordovaRecognition() {
        this._log('Setting up Cordova speech recognition', 'info');
    ...
    async _initializeBackgroundMode() {
        if (this.config.backgroundMode && cordova.plugins?.backgroundMode) {
            ...
            window.addEventListener('batterystatus', (status) => {
                this.state.voiceDetectionEnabled = status.level >= this.config.batteryThreshold;
    ...
    async _restartRecognition() {
        try {
            ...
            // Wait for any ongoing initialization to finish
            while (this.state.isProcessing) {
                await new Promise(resolve => setTimeout(resolve, 200));
            }

            // Add a cooldown before restarting
            await new Promise(resolve => setTimeout(resolve, 500)); // 500ms delay

            // Force re-initialization to create a new SpeechRecognition instance
    ...
    async startListening() {
        ...
        try {
            if (this.state.platform === 'Web') {
                ...
                // Attach a one-time error handler for "start" errors
                const recognition = this.state.recognition;
                let startErrorHandler;
                startErrorHandler = (event) => {
                    // Only handle errors thrown during .start()
                ...
                recognition.start();
                // Do NOT set isListening here; wait for onstart event
            } else {
                await this._startContinuousAudioProcessing();
                this.state.isListening = true;
            ...
            if (error && error.message && error.message.includes('recognition has already started')) {
