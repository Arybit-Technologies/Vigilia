...
function openEvidence() {
    // Show stored evidence (photos, audio, etc.)
    const evidence = window.vigiliaApp.getStoredData('evidence') || [];
    let html = '<ul class="list-group">';
    if (evidence.length === 0) {
        html += '<li class="list-group-item">No evidence stored yet.</li>';
    } else {
        evidence.forEach(item => {
            html += `<li class="list-group-item">
                <b>${item.type.replace('evidence_', '').toUpperCase()}</b> - ${new Date(item.timestamp).toLocaleString()}
                ${item.uri ? `<br><a href="${item.uri}" target="_blank">View</a>` : ''}
            </li>`;
        });
    }
    html += '</ul>';
    window.vigiliaApp.showModal('Evidence Vault', html);
}
...
function openLegalAid() {
    ...
    const html = `
        <ul>
            <li><a href="https://vigilia.co.ke/legal" target="_blank">Legal Help Center</a></li>
            <li><a href="tel:911">Call Emergency Legal Aid</a></li>
            <li><a href="mailto:legal@vigilia.com">Email Legal Support</a></li>
        </ul>
        <p class="text-muted">For urgent legal help, contact your local authorities.</p>
    `;
    window.vigiliaApp.showModal('Legal Aid', html);
}

function openSafeJourney() {
    // Show trip monitoring/check-in UI
    const html = `
        <p>Start a monitored journey and share your live location with trusted contacts.</p>
        <button class="btn btn-primary" onclick="startJourney()">Start Journey</button>
    `;
    window.vigiliaApp.showModal('Safe Journey', html);
}

function startJourney() {
    window.vigiliaApp.showStatus('🛣️ Journey monitoring started!', 'info');
    // Optionally, start location tracking and periodic check-ins
}

function openContacts() {
    // Show/manage emergency contacts
    const contacts = window.vigiliaApp.loadEmergencyContacts();
    let html = '<ul class="list-group">';
    contacts.forEach(c => {
        html += `<li class="list-group-item">${c.name} - ${c.phone || c.email || 'No contact info'}</li>`;
    });
    html += '</ul>';
    window.vigiliaApp.showModal('Emergency Contacts', html);
}

function openHealthInfo() {
    // Show stored medical info
    ...
    `;
    window.vigiliaApp.showModal('Medical Info', html);
}

function openCyberSafety() {
    // Show cyber safety tips/tools
    const html = `
        <ul>
            <li>Check app permissions regularly</li>
            <li>Beware of unsafe Wi-Fi networks</li>
            <li>Enable device encryption</li>
            <li>Use strong passwords</li>
        </ul>
        <p>More tools coming soon!</p>
    `;
    window.vigiliaApp.showModal('Cyber Safety', html);
}

function openSettings() {
    // Show settings modal
    window.vigiliaApp.showModal('Settings', '<p>Settings coming soon.</p>');
}

function startVideoRecording() {
    window.vigiliaApp.showStatus('🎥 Video recording not implemented in this demo.', 'warning');
}

// Expose functions globally for HTML onclick
window.openEvidence = openEvidence;
window.openLegalAid = openLegalAid;
window.openSafeJourney = openSafeJourney;
window.startJourney = startJourney;
window.openContacts = openContacts;
window.openHealthInfo = openHealthInfo;
window.openCyberSafety = openCyberSafety;
